...
func (s *SettingService) LoadForwardedClientIPSettings(ctx context.Context) error {
	...
	if values[settingKeyForwardedClientIPModeV2] != "true" {
		...
		// A stored value is operator-controlled. Do not reinterpret an explicit
		// false as a legacy default during migration; doing so can silently
		// re-enable trust in unsanitized forwarded headers.
	}
	if len(updates) > 0 {
