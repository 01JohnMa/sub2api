...
import { replaceVisibleUpstreamBrand } from '@/utils/siteBrand'
...
export const useAdminComplianceStore = defineStore('adminCompliance', () => {
  ...
  const expectedPhrase = computed(() =>
    replaceVisibleUpstreamBrand(
      rawExpectedPhrase.value,
      currentLocale.value === 'zh' ? FALLBACK_ZH_PHRASE : FALLBACK_EN_PHRASE
    )
  )
  ...
  async function accept(phrase: string): Promise<AdminComplianceStatus> {
    ...
    try {
      const submittedPhrase =
        phrase.trim() === expectedPhrase.value ? rawExpectedPhrase.value : phrase.trim()
